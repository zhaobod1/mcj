...
class uc_note
{
    ...
    function synlogin($get, $post)
    {
	    ...
	    if (!isset($row['user_id'])) {
	    	...
	    	$user=&init_users();
			$userInfo = $user->uc_call_func('uc_get_user',array('username'=>$username));
			hlog2("synlogin: userInfo: ");
			hlog2($userInfo);
			$insert=array();
			$insert['user_name']=$userInfo[1];
			$insert['email']=$userInfo[2];
			$insert['password']=$userInfo['password'];
			$insert['user_money']=$userInfo['user_money'];
			$insert['mobile_phone']=$userInfo['phone'];

		    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'),$insert, 'INSERT');
	    }
        ...
        set_login($row['user_id'], $row['user_name']);
    }
    ...
    function updatepw($get, $post)
    {
        ...
        if ($get['password']) {
	        ...
	        $sql="UPDATE " . $GLOBALS['ecs']->table('users') . " SET password='$password',ec_salt=NULL WHERE user_name='$username'";
	        $res=$this->db->query($sql);
	        hlog2('update user info: updatepw---->');
	        hlog2($sql);
	        hlog2($res);
        }


	    if (isset($get['user_money'])) {
		    $user_money=$get['user_money'];
		    $username=$get['username'];
		    $sql="update ".$GLOBALS['ecs']->table('users')." set user_money=" . $user_money . " where user_name='" . $username . "'";
		    hlog2('update user info: updatepw---->');
		    hlog2($sql);
		    $res = $this->db->query($sql);
		    hlog2($res);
	    }
	    if (isset($get['mobile_phone'])) {
		    $mobile_phone=$get['mobile_phone'];
		    $username=$get['username'];
		    $sql="update ".$GLOBALS['ecs']->table('users')." set mobile_phone='" . $mobile_phone . "' where user_name='" . $username . "'";
		    $res = $this->db->query($sql);
		    hlog2('update user info: updatepw---->');
		    hlog2($sql);
		    hlog2($res);
...
function set_login($user_id = '', $user_name = '')
{
    ...
    {
        $sql = "SELECT user_name, email, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id='$user_id' LIMIT 1";
        $row = $GLOBALS['db']->getRow($sql);
        if ($row)
        {
        	if (!isset($_COOKIE['ECS[user_id]']) && !isset($_COOKIE['ECS[username]'])) {
		        set_cookie($user_id, $row['user_name'], $row['email'],$row['password']);
		        set_session($user_id, $row['user_name'], $row['email'],$row['password']);
...
function set_cookie($user_id='', $user_name = '', $email = '',$password='')
{
    ...
    else
    {
        ...
        /* 火一五信息科技 huo15.com 设置cookie 日期：2017/5/9 */
	    setcookie("ECS[user_id]",  $user_id,   $time, $GLOBALS['cookie_path']."mobile", $GLOBALS['cookie_domain']);
	    setcookie("ECS[username]", $user_name, $time, $GLOBALS['cookie_path']."mobile", $GLOBALS['cookie_domain']);
	    setcookie("ECS[email]",    $email,     $time, $GLOBALS['cookie_path']."mobile", $GLOBALS['cookie_domain']);
	    setcookie("ECS[password]", $password,  $time, $GLOBALS['cookie_path']."mobile", $GLOBALS['cookie_domain']);

	    /* 火一五信息科技 huo15.com 设置cookie 日期：2017/5/9 end */
    }
}
 ...
 * @return void
 */
function set_session ($user_id = '', $user_name = '', $email = '',$password='')
{
    if (empty($user_id))
    {
