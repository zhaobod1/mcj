...
function log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
	...
	if ($res && UC_CONNECT) {
		...
		$arrUserInfo = $GLOBALS['db']->getRow($sql);
		$user = & init_users();
		$profile = array(
			"username"=>$arrUserInfo['user_name'],
			"user_money"=>$arrUserInfo['user_money'],
		);
		$user->edit_user($profile,1);
