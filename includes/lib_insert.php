 ...
 * 调用开通分站申请
 */
function insert_apply_sub($user){
    global $_CFG;
    //申请步骤
    $apply_info = array(0=>'sub',1=>'two_1',2=>'two_2',3=>'three',4=>'ing',5=>'four',6=>'fail');
    $sql = "select * from ".$GLOBALS['ecs']->table('substation_apply')." where state>-1 and member_id = ".$user['id'].' limit 1';
    $supplier = $GLOBALS['db']->getRow($sql);
   ...
   // $val = $GLOBALS['smarty']->fetch('library/apply_'.$apply_info[$shownum].'.lbi');
    if($shownum==1)
    {
        $val = $GLOBALS['smarty']->fetch('library/apply_sub.lbi');

    }else{
        $val = $GLOBALS['smarty']->fetch('library/apply_sub_'.$apply_info[$shownum].'.lbi');
    }
