...
class ucenter extends integrate
{
    ...
    function uc_call_func($method,$params) {
    	return uc_call($method, $params);
    }
    /**
     *  用户登录函数
     *
...
//             	$this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
    ...
    function edit_user($cfg, $forget_pwd = '0')
    {
    	 ...
    	 */
        $real_username = $cfg['username'];//用户名
        $cfg['username'] = addslashes($cfg['username']);//要保持到数据库的用户名
        $set_str = '';
    ...
    function remove_user($id)
    {
        ...
        //删除UCenter的会员
	    /* 火一五信息科技 huo15.com  批量删除会员 日期：2017/5/7 */
	    if (is_array($post_id)) {
	    	foreach ($post_id as $id) {
			    $info = uc_call('uc_get_user', $id);
			    if($info != 0){
				    $uid = $info[0];
				    // 删除UCenter
	    ...
	    /* 火一五信息科技 huo15.com  批量删除会员 日期：2017/5/7 end */
