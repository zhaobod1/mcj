...
class ucenter extends integrate
{
    ...
    function test($func, $arrParams) {
    	return uc_call($func,$arrParams);
    }
    /**
     *  用户登录函数
     *
    ...
    /*添加用户*/
    function add_user($username, $password, $email, $extra='')
    {
        /* 检测用户名 */
        ...
        else
        {
	        ...
	        if ($extra) {
		        if(isset($mobile_phone)){
			        ...
			        $sql='INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`email`, `mobile_phone`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`, `aite_id`, `froms`, `headimg`) VALUES ('$email', '$mobile_phone', '$username', '$password', '$reg_date', '$reg_date', '$ip', '$extra[aite_id]', '$extra[froms]', '$extra[headimg]')";
			        ...
			        $sql='INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`, `aite_id`, `froms`, `headimg`) VALUES ('$email', '$username', '$password', '$reg_date', '$reg_date', '$ip', '$extra[aite_id]', '$extra[froms]', '$extra[headimg]')";
		        ...
		        if(isset($mobile_phone)){
			        //$this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `mobile_phone`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$mobile_phone', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
			        $sql='INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`email`, `mobile_phone`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$email', '$mobile_phone', '$username', '$password', '$reg_date', '$reg_date', '$ip')";

		        }else{
			        //$this->db->query('INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`user_id`, `email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$uid', '$email', '$username', '$password', '$reg_date', '$reg_date', '$ip')");
			        $sql='INSERT INTO ' . $GLOBALS['ecs']->table("users") . "(`email`, `user_name`, `password`, `reg_time`, `last_login`, `last_ip`) VALUES ('$email', '$username', '$password', '$reg_date', '$reg_date', '$ip')";
    ...
    function edit_user($cfg, $forget_pwd = '0')
    {
        ...
        $valarr =array('email'=>'email', 'gender'=>'sex', 'bday'=>'birthday' , 'password'=>'password', 'mobile_phone'=>'mobile_phone','mobile_validated'=>'mobile_validated','username'=>'username','user_money'=>'user_money');
	    ...
	    if (!empty($cfg['mobile_phone'])) {
		    //($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '',$extra='')
		    $ucresult = uc_call("uc_user_edit", array($real_username, '', '', '', 1,'','',array(
			    "mobile_phone"=>$cfg['mobile_phone']
		    )));

		    hlog2("更新 phone： ". $ucresult);

	    }
	    if (!empty($cfg['user_money'])) {
		    //($username, $oldpw, $newpw, $email, $ignoreoldpw = 0, $questionid = '', $answer = '',$extra='')
		    $ucresult = uc_call("uc_user_edit", array($real_username, '', '', '', 1,'','',array(
			    "user_money"=>$cfg['user_money']
		    )));

		    hlog2("更新 user_money： ". $ucresult);
