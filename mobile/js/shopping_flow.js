...
/* *
 * 检查分站提交是否正确 by Jason Ding
 */
function checkSubConfig(frm)
{
    var msg = new Array();
    var err = false;
    ...
    {
        err = true;
        msg.push(sub_province_not_null);
    }

    if (frm.elements['city'] && frm.elements['city'].value == 0 && frm.elements['city'].length > 1)
    {
        err = true;
        msg.push(sub_city_not_null);
    }
    ...
    if (frm.elements['district'] && frm.elements['district'].length > 1)
    {
        ...
        {
            err = true;
            msg.push(sub_district_not_null);
        }
    }
    ...
    {
        err = true;
        msg.push(sub_consignee_not_null);
    }
    if ( frm.elements['email'].value !=='' && ! Utils.isEmail(frm.elements['email'].value))
    {
        err = true;
        msg.push(sub_invalid_email);
    }

    if (frm.elements['address'] && Utils.isEmpty(frm.elements['address'].value))
    {
        err = true;
        msg.push(sub_address_not_null);
    }

    if (frm.elements['zipcode'] && frm.elements['zipcode'].value.length > 0 && (!Utils.isNumber(frm.elements['zipcode'].value)))
    {
        err = true;
        msg.push(sub_zip_not_num);
    }

    if (frm.elements['mobile'] && (Utils.isEmpty(frm.elements['mobile'].value)))
    {
        err = true;
        msg.push(sub_mobile_not_null);
    }
    else
    {
        if (frm.elements['mobile'] && !Utils.isMobile(frm.elements['mobile'].value))
