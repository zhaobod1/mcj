...
class usercontrol extends base {
	...
	function onedit() {
		...
		$status = $_ENV['user']->edit_user($username, $oldpw, $newpw, $email, $ignoreoldpw, $questionid, $answer, $extra);
		...
		if(count($extra) && !$newpw) {
			$this->load('note');
			$urlParam='username='.urlencode($username)."&";
			foreach ($extra as $key=>$value) {
				$urlParam.=$key."=".urlencode($value)."&";
			}
			$_ENV['note']->add('updatepw', $urlParam);

			/* 火一五信息科技 huo15.com 解决通知不同步问题 日期：2017/5/8 */
			fixSync();
			/* 火一五信息科技 huo15.com 解决通知不同步问题 日期：2017/5/8 end */
	...
	function onget_user() {
		...
		if($status) {
			...
			$status2[]=$status['uid'];
			unset($status['uid']);
			$status2[]=$status['username'];
			unset($status['username']);
			$status2[]=$status['email'];
			unset($status['email']);
