...
function action_sub_apply()
{
    ...
    // 判断 该用户是否是分站
    $sql = "select * from ".$GLOBALS['ecs']->table('substation_apply')." where state>-1 and member_id = ".$user_id.' limit 1';
    $supplier = $GLOBALS['db']->getRow($sql);
    if($supplier){
        if($supplier['state']==13)
        {
            // 删除已提交的记录，
            $sql = 'UPDATE ' . $ecs->table('substation_apply') . ' SET state =-1 '.
                "WHERE member_id = '" . $user_id . "'";
            $db->query($sql);
            show_message('分站申请未通过', '重新申请', '/');
        }else if($supplier['state']==12)
        {
            show_message('分站申请已通过', '返回首页', '/');
            exit;
        }else {
            show_message('分站申请未审核', '返回首页', '/');
        }
    }
	...
	$smarty->display('sub_user_transaction.dwt');
}
...
/* 添加分站申请处理 by Jason -五月风软件工作室 */
function action_act_sub_apply()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    $save['full_name'] = isset($_POST['consignee']) ? trim(addslashes(htmlspecialchars($_POST['consignee']))) : '';
    $save['telephone'] = isset($_POST['mobile']) ? trim(addslashes(htmlspecialchars($_POST['mobile']))) : '';
    $save['email'] = isset($_POST['email']) ? trim($_POST['email']) : '';
    $save['level'] = isset($_POST['sub_level']) ? trim($_POST['sub_level']) : 1;
    ...
    if(count($save1)!=count($save)){
        show_message('请认真填写必填申请资料！', '返回', 'user.php?act=sub_apply', 'wrong');
    }

// 验证是否可以申请该地区分站 by jason
    ...
    if($db->autoExecute($ecs->table('substation_apply'),$save)!==false)
    {
        ...
        show_message('分站申请已提交', '返回首页', '/');
        exit;
    }else{
        show_message('操作失败！', '返回', 'user.php?act=sub_apply', 'wrong');
    }
