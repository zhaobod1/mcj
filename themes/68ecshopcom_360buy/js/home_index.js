...
$(function () {
  setTimeout("takeCount()", 1000);
  console.log("tab-nav.length:");
  console.log($(".tabs-nav").length);
  $(".tabs-panel").each(function (index, element) {
    if (index == 0) {
      $(element).removeClass("tabs-hide").addClass("tabs-hide");
    }
    if (index == 1) {
      $(element).removeClass("tabs-hide");
    }
  ...
  $(".floor-tabs-nav > li").bind('mouseover', (function (e) {
    ...
    $(this).find('h3').css({
      ...
      'color': color
    }).parents('li').siblings('li').find('h3').css({'border-color': '', 'color': ''});
