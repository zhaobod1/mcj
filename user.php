...
/*余额额支付密码_更改_START_www.68ecshop.com*/
'act_forget_pass', 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code','sub_order_list','sub_supplier_list','sub_member_list'
); // 代码修改
   // By
   // www.68ecshop.com
/* 余额额支付密码_更改_END_www.68ecshop.com */
...
$subAuth=$db->getRow("SELECT province,city,district,level FROM " . $GLOBALS['ecs']->table('substation_apply') . " WHERE state=12 and member_id= '$user_id'");
if($subAuth)
{
	$smarty->assign('sub_auth', 1);
}else{
	$smarty->assign('sub_auth', 0);
}
...
//更新最新订单收益
function updateSubOderProfit()
{
    // 先查询 订单收益的最晚时间，然后以该时间往后查询，并更新 user_money
   //查询用户的分站等级 及省市区
    $db = $GLOBALS['db'];
    $ecs = $GLOBALS['ecs'];
    $user_id = $_SESSION['user_id'];
    $subAuth=$db->getRow("SELECT province,city,district,level,profit FROM " . $GLOBALS['ecs']->table('substation_apply') . " WHERE state=12 and member_id= '$user_id'");

    // 查询今日入驻商户
    $level=0;
    $levelWhere='';
    $subAuth and $level=(int)$subAuth['level'];
    if($level==3)
    {
        $levelWhere='and province='.(int)$subAuth['province'];
    }else if($level==2)
    {
        $levelWhere='and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];
    }else if($level==1)
    {
        $levelWhere='and district='.(int)$subAuth['district'].' and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];;
    }else {
        return false;
    }
    //查询account—log 的最近时间，分站入账标志设为101
    $newChangeTime=$db->getOne("SELECT change_time FROM " . $GLOBALS['ecs']->table('account_log') . " WHERE change_type=101 and user_id= '$user_id' order by change_time desc");

    $listSql="SELECT * FROM " . $ecs->table('sub_order_view') . " WHERE add_time>'$newChangeTime' ".$levelWhere;

    $orderList = $GLOBALS['db'] -> selectLimit($listSql, 100, 0);
    while ($row = $GLOBALS['db']->fetchRow($orderList))
    {

        $totalFee=$row['goods_amount'] + $row['shipping_fee'] + $row['insure_fee'] + $row['pay_fee'] + $row['pack_fee'] + $row['card_fee'] + $row['tax'] - $row['discount'];
        // 写入 account_log 表,pay_points 字段用于保存订单号，订单号不能重复
        $change_time=$row['add_time'];
        $change_desc='分站入账 订单号-'.$row['order_sn'];
        $change_type=101;
        $user_money=sprintf("%.2f", $subAuth['profit']*$totalFee/100);
        $pay_points=$row['order_id'];
        // 查询 某个 订单号是否存在
        $isExist = $db->GetOne('SELECT pay_points FROM ' . $ecs->table('account_log') . " WHERE user_id='$user_id' and change_type=101 and pay_points=".$pay_points);
        if($isExist)
        {
           break;
        }

        $sql = "INSERT INTO " . $ecs->table('account_log') . "(user_id, user_money, change_time, change_desc,change_type,pay_points) VALUES ('" . $user_id . "', '" . $user_money . "', '$change_time', '" . $change_desc . "','".$change_type."','".$pay_points."')";
        $result=$db->query($sql);
        // 更新用户表中的 user_money
        if($result)
        {
            $sql = 'UPDATE ' . $ecs->table('users') . " SET user_money = user_money+'$user_money' WHERE user_id = '$user_id'";
            $res=$db->query($sql);
        }
...
// 查看分站 商户列表
function action_sub_supplier_list()
{
	$user = $GLOBALS['user'];
	$_CFG = $GLOBALS['_CFG'];
	$_LANG = $GLOBALS['_LANG'];
	...
	$composite_status=isset($_GET['composite_status'])?$_GET['composite_status']:0;
    ...
    {
        $levelWhere='and province='.(int)$subAuth['province'];
        $sub_name=get_region_name($subAuth['province']).' 分站';

    }else if($level==2)
    {
        $sub_name=get_region_name($subAuth['province']).'-'.get_region_name($subAuth['city']).' 分站';
        $levelWhere='and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];
    }else if($level==1)
    {
        $levelWhere='and district='.(int)$subAuth['district'].' and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];;
        $sub_name=get_region_name($subAuth['province']).'-'.get_region_name($subAuth['city']).'-'.get_region_name($subAuth['district']).' 分站';
    ...
    //$startTime=1437526850;
    $endTime=$startTime+86400;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('supplier') . " WHERE add_time>'$startTime' and add_time<'$endTime' and status=1 ".$levelWhere);
    $all_count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('supplier') . " WHERE status=1 ".$levelWhere);
    $supp_list = array();
    if($composite_status)
    {
        ...
        $supplierListSql="SELECT * FROM " . $ecs->table('supplier') . " WHERE status=1 ".$levelWhere;
    }else{
        $pager = get_pager('user.php', array(
            'act' => $action,
            'composite_status'=>0
        ), $record_count, $page);
        $supplierListSql="SELECT * FROM " . $ecs->table('supplier') . " WHERE add_time>'$startTime' and add_time<'$endTime' and status=1 ".$levelWhere;

    }
    $supplierList = $GLOBALS['db'] -> selectLimit($supplierListSql, $pager['size'], $pager['start']);
    while ($row = $GLOBALS['db']->fetchRow($supplierList))
    {
        $one=array();
        $one['supplierid']  = $row['supplier_id'];
        $one['supplier_name']= $row['supplier_name'];
        $one['tel']= $row['contacts_phone'];
        $one['company_name']= $row['company_name'];
        $one['url']= "supplier.php?go=index&suppId=".$row['supplier_id']."&id=0";
        $one['shop_name']= $row['shop_name'];
        $one['qq']= $row['qq'];
        $one['ww']= $row['ww'];
        $suppinfo = $GLOBALS['db'] -> query("select value,code from " . $GLOBALS['ecs']->table('supplier_shop_config') ." where supplier_id = ".$row['supplier_id']." AND code in('shop_name','shop_logo','qq','ww')");
	...
	$smarty->display('user_sub_clips.dwt');
}

// 查看分站 会员列表
function action_sub_member_list()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
    ...
    $user_id = $_SESSION['user_id'];
    $action = $GLOBALS['action'];
    $composite_status=isset($_GET['composite_status'])?(int)$_GET['composite_status']:0;
    include_once (ROOT_PATH . 'includes/lib_clips.php');
    ...
    //$startTime=1437526850;
    $endTime=$startTime+86400;
    $record_count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE reg_time>'$startTime' and reg_time<'$endTime' ".$levelWhere);
    $all_count = $db->getOne("SELECT COUNT(*) FROM " . $ecs->table('users') . " WHERE 1 ".$levelWhere);
    $member_list = array();
    if($composite_status)
    {
        $pager = get_pager('user.php', array(
        ...
        $memberListSql="SELECT * FROM " . $ecs->table('users') . " WHERE 1 ".$levelWhere;
    }else{
        $pager = get_pager('user.php', array(
            'act' => $action,
            'composite_status'=>0
        ), $record_count, $page);
        $memberListSql="SELECT * FROM " . $ecs->table('users') . " WHERE reg_time>'$startTime' and reg_time<'$endTime' ".$levelWhere;

    }

    $memberList = $GLOBALS['db'] -> selectLimit($memberListSql, $pager['size'], $pager['start']);
    include_once (ROOT_PATH . 'includes/lib_order.php');
    while ($row = $GLOBALS['db']->fetchRow($memberList))
    {
        $one=array();
        $one['user_name']=$row['user_name'];
        $one['reg_time']=date('Y-m-d H:i:s',$row['reg_time']);
        $one['address']=$row['address'];
        $one['address']=$row['address'];
        $one['mobile_phone']=$row['mobile_phone'];
        $one['province']=get_region_info($row['province']);
        $one['city']=get_region_info($row['city']);
        $one['headimg']=$row['headimg'];
        $one['district']=get_region_info($row['district']);
        $member_list[]=$one;
    }
    $smarty->assign('sub_name', $sub_name);
    $smarty->assign('pager', $pager);
    $smarty->assign('new_count', (int)$record_count);
    $smarty->assign('all_count', (int)$all_count);
    $smarty->assign('composite_status', $composite_status);
    ...
    $smarty->assign('lang_list', $lang_list);
    $smarty->assign('user_id', $user_id);
	$smarty->display('sub_member_list.dwt');
}

// 查看分站订单 by JASON Ding

function action_sub_order_list()
{
    $user = $GLOBALS['user'];
    $_CFG = $GLOBALS['_CFG'];
    $_LANG = $GLOBALS['_LANG'];
   ...
   // 查询今日入驻商户
    ...
    {
        $levelWhere='and province='.(int)$subAuth['province'];
        $memberWhere='and receiver_province='.(int)$subAuth['province'];
        $sub_name=get_region_name($subAuth['province']).' 分站';
    }else if($level==2)
    {
        $levelWhere='and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];
        $memberWhere='and receiver_city='.(int)$subAuth['city'].' and receiver_province='.(int)$subAuth['province'];
        $sub_name=get_region_name($subAuth['province']).'-'.get_region_name($subAuth['city']).' 分站';

    }else if($level==1)
    {
        $levelWhere='and district='.(int)$subAuth['district'].' and city='.(int)$subAuth['city'].' and province='.(int)$subAuth['province'];
        $memberWhere='and receiver_district='.(int)$subAuth['district'].' and receiver_city='.(int)$subAuth['city'].' and receiver_province='.(int)$subAuth['province'];
        $sub_name=get_region_name($subAuth['province']).'-'.get_region_name($subAuth['city']).'-'.get_region_name($subAuth['district']).' 分站';
    ...
    // 关联表 supplier ，order_info,order_goods
    $startTime=strtotime(date('Y-m-d',time()));
    $endTime=$startTime+86400;
    $today= $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$startTime ." and add_time<".$endTime.' and pay_status=2 '.$levelWhere);
    // 本月订单总数
   // $beginThismonth=strtotime(date('Y').'-'.date('m').'-1');
   // $endThismonth=time();

     $beginThismonth=$startTime;
     $endThismonth=$endTime;
     $month = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$beginThismonth ." and add_time<".$endThismonth. ' and pay_status=2 '.$levelWhere);
    // 今日订单总和
	//$todaySum= $db->getRow('SELECT SUM(goods_amount) as goods_amount , SUM(shipping_fee) as shipping_fee, SUM(insure_fee) as insure_fee, SUM(pay_fee) as pay_fee,SUM(pack_fee) as pack_fee,sum(card_fee) as card_fee, sum(tax) as tax, sum(discount) as discount FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$startTime ." and add_time<".$endTime.' and pay_status=2');
	$todaySum= $db->GetOne('SELECT (SUM(goods_amount) + SUM(shipping_fee)+SUM(insure_fee) + SUM(pay_fee)+ SUM(pack_fee)+SUM(card_fee) + SUM(tax) - SUM(discount) ) as total_sum FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$startTime ." and add_time<".$endTime.' and pay_status=2 '.$levelWhere);
    //本月订单总和
	$monthSum= $db->GetOne('SELECT (SUM(goods_amount) + SUM(shipping_fee)+SUM(insure_fee) + SUM(pay_fee)+ SUM(pack_fee)+SUM(card_fee) + SUM(tax) - SUM(discount) ) as total_sum FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$beginThismonth ." and add_time<".$endThismonth. ' and pay_status=2 '.$levelWhere);

	// 获得本地会员订单总和
    $member = $db->GetOne('SELECT COUNT(*) FROM ' . $ecs->table('sub_order_view') . " WHERE  pay_status=2 ".$memberWhere);

    $smarty->assign('today', $today);
    $smarty->assign('month', $month);
    $smarty->assign('today_sum', $todaySum);
    $smarty->assign('month_sum', $monthSum);
    ...
    switch($composite_status)
    {
        ...
        case 0:
        case 101:
         $listSql="SELECT * FROM " . $ecs->table('sub_order_view') . " WHERE add_time>'$startTime' and add_time<'$endTime' ".$levelWhere;
         $record_count=$today;
        break;
        //本月订单
        case 100:
        case 102:
          $record_count=$month;
          $listSql = 'SELECT * FROM ' . $ecs->table('sub_order_view') . " WHERE 1 and add_time>".$beginThismonth ." and add_time<".$endThismonth. ' and pay_status=2 '.$levelWhere;
        break;
        case 103:
            $record_count=$member;
            $listSql = 'SELECT * FROM ' . $ecs->table('sub_order_view') . " WHERE  pay_status=2 ".$memberWhere;
            break;
    ...
    $order_list=array();
    $orderList = $GLOBALS['db'] -> selectLimit($listSql, $pager['size'], $pager['start']);

    while ($row = $GLOBALS['db']->fetchRow($orderList))
    {

        $one=array();
        $one['order_id']  = $row['order_id'];
        $one['order_sn']= $row['order_sn'];
        $one['shopname']= $row['supplier_name'];
        $one['supplier_id']= $row['supplier_id'];
        $one['goods_name']= $row['goods_name'];
        $one['goods_attr']= $row['goods_attr'];
        $one['order_time']= date('Y-m-d H:i:s',$row['add_time']);
        $one['is_suborder']= $row['is_suborder'];
        $one['goods_thumb']= $row['goods_thumb'];
        $one['goods_number']= $row['goods_number'];
        $one['goods_price']= $row['goods_price'];
        $one['goods_id']= $row['goods_id'];
        $one['consignee']= $row['consignee'];
        $one['receiver_address']= $row['receiver_address'];
        $totalFee=$row['goods_amount'] + $row['shipping_fee'] + $row['insure_fee'] + $row['pay_fee'] + $row['pack_fee'] + $row['card_fee'] + $row['tax'] - $row['discount'];
        $one['url']= 'goods.php?id='.$row['goods_id'];
        $one['total_fee']= sprintf("%.2f", $totalFee);
        $one['order_profit']= sprintf("%.2f", $subAuth['profit']*$totalFee/100);
//        $one['qq']= $row['qq'];
//        $one['ww']= $row['ww'];
       $order_list[]=$one;
    }
    ...
    $smarty->assign('composite_status', $composite_status);
    $smarty->display('user_transaction_sub.dwt');
}
